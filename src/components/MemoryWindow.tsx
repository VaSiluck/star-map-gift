...
const BLOCK_DEFS: { key: keyof StarBlocks; label: string; hint: string }[] = [
  { key: "date", label: "дата", hint: "дата / период в шапке окна" },
  { key: "photo", label: "фото", hint: "фотография вверху окна" },
  { key: "caption", label: "подпись", hint: "подпись под фото" },
  { key: "story", label: "как было", hint: "главный текст воспоминания" },
  { key: "storyLabel", label: "заголовок", hint: "надпись «Как это было» над текстом" },
  { key: "gift", label: "подарок", hint: "блок «Что ты мне принесла»" },
  { key: "thought", label: "цитата", hint: "строка-мысль внизу" },
...
export default function MemoryWindow({
  ...
  const { editMode, updateStar } = useData();
  const dragRef = useRef<{ dx: number; dy: number } | null>(null);
  const [imgLoaded, setImgLoaded] = useState(false);

  const blocks = star.blocks ?? {};
  /** блок виден, если в данных явно не выключен (undefined = показан) */
  const on = useCallback((k: keyof StarBlocks) => blocks[k] !== false, [blocks]);

  const has = (k: keyof StarBlocks) => {
    switch (k) {
      case "date":
        return !!star.date;
      case "photo":
  ...
  const toggleBlock = useCallback(
    (k: keyof StarBlocks) => {
      updateStar(star.id, { blocks: { ...(star.blocks ?? {}), [k]: blocks[k] === false } });
    },
    [blocks, star.blocks, star.id, updateStar],
  );
  ...
  const showPhoto = on("photo") && !!star.photo;
        ...
        <>
          <div className="thin-scroll relative flex-1 overflow-y-auto overscroll-contain">
            ...
            <div className="space-y-4 px-4 py-4">
              {/* подпись без фото — просто курсивная строка */}
              {!showPhoto && on("caption") && star.caption && (
                <p className="font-display text-[13.5px] italic leading-snug text-white/70">{star.caption}</p>
              )}
          ...
          {/* живая настройка состава окна в режиме автора */}
          {editMode && (
            <div className="relative flex shrink-0 flex-wrap items-center gap-1 border-t border-amber-200/15 bg-[#0a0c1c]/85 px-2 py-2">
              <span className="mr-0.5 text-[8px] uppercase tracking-[0.18em] text-amber-100/40">окно:</span>
              {BLOCK_DEFS.map((b) => {
                const visible = on(b.key);
                const locked = !has(b.key) || (b.key === "storyLabel" && !on("story"));
                return (
                  <button
                    key={b.key}
                    onClick={() => toggleBlock(b.key)}
                    disabled={locked}
                    title={locked ? `${b.hint} — пока недоступно` : b.hint}
                    className={`rounded-full border px-2 py-0.5 text-[10px] leading-none transition ${
                      visible
                        ? "border-amber-300/50 bg-amber-300/15 text-amber-100"
                        : "border-white/10 bg-white/[0.04] text-white/35 hover:text-white/60"
