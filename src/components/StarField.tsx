...
export default function StarField({
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (d.moved && onDragStar) {
        const wx = camera.x + (e.clientX - vw / 2) / camera.zoom;
        const wy = camera.y + (e.clientY - vh / 2) / camera.zoom;
  ...
  }, [editMode, camera.x, camera.y, camera.zoom, vw, vh, onDragStar]);

  // Полярную показываем в режиме автора всегда (если она включена), а для зрителя — после всех звёзд
